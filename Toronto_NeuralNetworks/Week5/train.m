...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    embed_to_hid_weights_gradient = embedding_layer_state * back_propagated_deriv_1'; % (b) zeros(numhid1 * numwords, numhid2);
    % Options:
    ...
    % FILL IN CODE. Replace the line below by one of the options.
    hid_bias_gradient = sum(back_propagated_deriv_1, 2); % (a) % zeros(numhid2, 1);
    % Options
    ...
    % FILL IN CODE. Replace the line below by one of the options.
    back_propagated_deriv_2 = embed_to_hid_weights * back_propagated_deriv_1; % (a) % zeros(numhid2, batchsize);
    % Options
