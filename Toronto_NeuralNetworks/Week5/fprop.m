...
hidden_layer_state = 1 ./ (1 + exp(-inputs_to_hidden_units));  % (c) % zeros(numhid2, batchsize);
% Options
...
% FILL IN CODE. Replace the line below by one of the options.
inputs_to_softmax = hid_to_output_weights' * hidden_layer_state +  repmat(output_bias, 1, batchsize); % (a) % zeros(vocab_size, batchsize);
% Options
